...
def finetune_cql(base_model, episodes, model_params, n_steps, actions_dict, experiment_name):
    ...
    # freeze the encoder
    for name, param in new_cql._impl._checkpointer.modules['q_funcs'].named_parameters():
        if 'encoder' in name:
            param.requires_grad = False
    for name, param in new_cql._impl._checkpointer.modules['targ_q_funcs'].named_parameters():
        if 'encoder' in name:
            param.requires_grad = False
...
if __name__ == '__main__':
    ...
    result_filename = os.path.join(trc.EXPERIMENTS_RESULTS, stratify_on, f"{stratify_on}_evaluate_on_finetuned_vlowerLRFrozenEncoder_{major_group}_CQL_policy_{datetime.now().strftime('%Y%m%d%H%M%S')}.csv")
